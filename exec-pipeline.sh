...
trap "pkill -9 -f 'bedrock'; pkill -9 -f 'streamer-daq'; pkill -9 -f 'streamer-dist'; pkill -9 -f 'streamer-sirt'; pkill -9 -f 'streamer-den'; exit 1" SIGINT SIGTERM EXIT
...
sleep 10
echo "Start DAQ ------------------------------------------------------------"
bash run-daq.sh ${sirt_ranks} ${num_sinograms} > ${logdir}/daq.out 2> ${logdir}/daq.err &
sleep 10
echo "Start DIST -----------------------------------------------------------"
bash run-dist.sh ${num_sinograms} > ${logdir}/dist.out 2> ${logdir}/dist.err &
echo "Start SIRT -----------------------------------------------------------"
bash run-sirt.sh ${sirt_ranks} ${logdir} > ${logdir}/sirt.out 2> ${logdir}/sirt.err &
echo "Start DEN ------------------------------------------------------------"
bash run-den.sh ${sirt_ranks} 2> ${logdir}/den.err | tee ${logdir}/den.out

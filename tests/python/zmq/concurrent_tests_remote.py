...
parser.add_argument("-cmd", "--command", dest="cmd",
  help="Full path of the zmq performance command, e.g. \'/home/bicer/projects/zeromq-4.2.0/perf/remote_thr\'")
...
msize = args.msize
mnum = args.mnum
cmd = args.cmd # '/home/bicer/projects/zeromq-4.2.0/perf/remote_thr' # I'm client

iport = args.iport #50000 # starting port number
pnum = args.pnum #4 # num parallel transfers

#address = '164.54.113.157' # mona4 (server)
address = args.address #'164.54.113.150' # mona3 (server)


def init_transfer(cmd, remote_address, port, msg_size, msg_num):
  remote = "tcp://{}:{}".format(remote_address, port)
  print("Address: {}; msg_size={}; msg_num={}; total(MiB)={}".format(remote, msg_size, msg_num, msg_size*msg_num/(1024*1024.)))

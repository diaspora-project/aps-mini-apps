...
mpiexec --no-vni -ppn $num_node_mofka --hosts $node_mofka -n $num_node_mofka bash $exec_dir/run-mofka-polaris.sh > "${logdir}/mofka.out" 2> "${logdir}/mofka.err" &
...
echo "Start DIST -----------------------------------------------------------"
mpiexec --no-vni -ppn 1 -d 16 --hosts $node_dist bash $exec_dir/run-dist.sh "${num_sinograms}" "${sirt_tasks}" ${load_balance} "${logdir}" > "${logdir}/dist.out" 2> "${logdir}/dist.err" &
# bash $exec_dir/run-dist.sh "${num_sinograms}" "${sirt_tasks}" "${logdir}" > "${logdir}/dist.out" 2> "${logdir}/dist.err" &
echo mpiexec --no-vni -ppn 1 -d 16 --hosts $node_dist bash $exec_dir/run-dist.sh "${num_sinograms}" "${sirt_tasks}" "${logdir}"
...
echo "Start DEN ------------------------------------------------------------"
echo mpiexec --no-vni -ppn 1 -d 16 --hosts $node_den bash $exec_dir/run-den.sh "${sirt_tasks}" "${logdir}"
# IMPORTANT: DEN is the foreground block until pipeline finishes
mpiexec --no-vni -ppn 1 -d 16 --hosts $node_den bash $exec_dir/run-den.sh "${sirt_tasks}" "${logdir}" 2> "${logdir}/den.err" | tee "${logdir}/den.out"

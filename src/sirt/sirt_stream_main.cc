...
//           times.push_back(elapsed.count());  // Store elapsed time
//           std::cout << "Size: " << sizeof(float)*rank_dims[0]*rank_dims[1]*rank_dims[2] << ", Time: " << elapsed.count() << " sec" << std::endl;
//           outFile << sizeof(float)*rank_dims[0]*rank_dims[1]*rank_dims[2] << "," << elapsed.count() << "\n";
//           if (c==2) {
//             producer.flush();
//             c =0;
//           }
...
//   }
//   outFile.close();
//   std::cout << "Results saved to timing_results.csv" << std::endl;
...
class MofkaStream
{
  private:
    ...
    mofka::BatchSize   batchSize   = mofka::BatchSize{batchsize}; //mofka::BatchSize{1};
    mofka::ThreadCount threadCount = mofka::ThreadCount{1};
    mofka::Ordering    ordering    = mofka::Ordering::Strict;
  ...
  public:
    ...
    void PublishImage(json &j, float *data, size_t size, mofka::Producer producer){
      mofka::Metadata metadata{j};
      ...
      if (batch == batchsize){
        ...
        elapsed = end - start;
        std::cout << "Flush " << batch << "Time: " << elapsed.count() << " sec" << std::endl;
  ...
  for(int passes=0; ; ++passes){
      ...
      if(!(passes%config.write_freq)){
        ...
        try {
          ...
          auto start = std::chrono::high_resolution_clock::now();
          ms.PublishImage(md, &recon[recon_slice_data_index], rank_dims[0]*rank_dims[1]*rank_dims[2], producer);
          auto end = std::chrono::high_resolution_clock::now();
          std::chrono::duration<double> elapsed = end - start;
          // mofka::Metadata metadata{md};

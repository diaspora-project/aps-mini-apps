...
class TDataMock
{
  ...
  public:
    TDataMock(
      ...
      vdata_{new std::vector<float>(num_projs_*num_slices_*num_cols_)},
      vtheta_{new std::vector<float>(num_cols_)}
    { }
    ...
    std::vector<float>& projs() const { return *vdata_; };
    std::vector<float>& theta() const { return *vtheta_; };
};
...
int main(int argc, char **argv)
{
  ...
  TraceMetadata trace_metadata(
      static_cast<float *>(mock_data.theta().data()),  /// float const *theta,
      0,                                  /// int const proj_id,

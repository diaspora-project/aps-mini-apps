...
print(f"Loaded slowdown from: {input_file}", slowdown.shape)
print("Dtype:", slowdown.dtype)

# Example usage
print("Mean of set 1:", slowdown[1].mean())
print("First value:", slowdown[1, 0])

# ---------------------------------------------------------------------------
# Parsl app
# ---------------------------------------------------------------------------
@bash_app
def run_sirt(id, slowdownindex, logdir=".", args=None, launcher_env=None, sirt_bin_path=""):
    args = args or []
    ...
    cmd = " && ".join(
        env_exports
        + diag
        + [f'{sirt_bin_path} --worker-id {id} --slowdown {slowdown[slowdownindex, int(id)]} ' + " ".join(args) + f' >> "{stdout}" 2>> "{stderr}"']
    )
    return cmd
...
def main():
    ...
    helper_keys = {"extra_ld_paths", "sirt_bin", "slowdownindex"}
    all_args = []

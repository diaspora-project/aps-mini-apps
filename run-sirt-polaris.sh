...
if [ "$#" -ne 6 ]; then
    ...
    echo "Usage: run-sirt.sh <number of processes> <logdir> <slowdown> <ckpt_freq> <mttf> <failure_mode>"
    exit 1
fi
...
bash $exec_dir/run-exp-control.sh "${failure_mode}" "${mttf}" "${logdir}" 2> "${logdir}/exp-control.err" | tee "${logdir}/exp-control.out" &
...
cd ./build/python/streamer-sirt

python -u ParslSirtPolaris.py \
